...
void breadthFirst (tnode_t* root) {
  ...
  queue_t* tree_queue = newQueue();
  // put root in the queue
  enqueue(tree_queue,root);
  while (!isEmpty(tree_queue)){
    // visit the first element of the queue
    tnode_t* temp_node = dequeue(tree_queue);
    printf("%c",temp_node->data);
    // if current node has left child, put at the tail of the queue
    if(temp_node->left != NULL) {
      enqueue(tree_queue,temp_node->left);
    }
    // if current node has right child, put at the tail of the queue
    if(temp_node->right != NULL){
      enqueue(tree_queue,temp_node->right);
    }
   
  }
  freeQueue(tree_queue);
