...
void mergeIt(
  ...
  int i = leftStart;
  int j = rightStart;
  //create a new char array with the same size 
  char arr[rightStop - leftStart+1];
  //compare the start elements from both side and put the smaller one infront 
  ...
  // copy the left elements from the left side
  while (i <= leftStop){
    arr[temp] = data[i];
    i++;
    temp++;
  }
  // copy the left elements from the right side
  while (j <= rightStop){
    arr[temp] = data[j];
    j++;
    temp++;
}
  // copy the sorted elements from arr to data
  int k;
  for (k = 0; k < rightStop-leftStart+1; k++){
    data[leftStart+k] = arr[k];
  }
  return;
}
